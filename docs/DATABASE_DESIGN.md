...
Pro is in high demand at the moment
Another model was used for this response. This didn't count towards your limit.
...
| Entity 1 | Relationship | Entity 2 | Description / Business Rule |
| :--- | :--- | :--- | :--- |
| **ApplicationUser** | 1 to 1 | **StudentProfile** | Every registered student has one profile. |
| **CourseCategory** | 1 to Many | **Course** | A category has many courses; a course belongs to one category. |
| **Course** | 1 to Many | **CourseSyllabus** | A course has many syllabus modules. If course is deleted, modules are deleted (Cascade). |
| **Course** | 1 to Many | **Batch** | A course has many batches. |
| **Instructor** | 1 to Many | **Batch** | An instructor can teach many batches. A batch has one instructor (can be null initially). |
| **ApplicationUser** | 1 to Many | **Enrollment** | A student can have many enrollments. |
| **Batch** | 1 to Many | **Enrollment** | A batch contains many enrolled students. |
| **Enrollment** | 1 to Many | **Payment** | An enrollment can have multiple payment records (installments). |

---

## 3. Business Logic Notes for Database
...
Displaying docs/DATABASE_DESIGN.md.