...
│   └── DbInitializer.cs       # Seed roles & admin user
│
├── Models/                    # Domain Layer
│   ├── Entities/              # DB Tables (Course, Batch, Enrollment)
│   └── ViewModels/            # UI Data shapes (CourseDetailsVM, DashboardVM)
│
...
├── appsettings.json           # Config (DB Connection)
└── ITTrainingCenter.csproj
```

## 3. Data Flow (Request Lifecycle)
Here is exactly how a request flows through the system when a Student enrolls in a course:

1.  **User Clicks "Enroll" (View):** Browser sends HTTP POST to `/Courses/Enroll/5`.
...
*   **Clean Controllers:** Controllers should only manage HTTP routing and View selection. They shouldn't know how a payment is calculated.
*   **Testability:** In the future, you can write Unit Tests against `IPaymentService` without needing a real database or HTTP context.
High_Level_Architecture_Document.md
Displaying High_Level_Architecture_Document.md.